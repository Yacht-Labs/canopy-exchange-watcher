...
const vaultContract = new web3Polygon.eth.Contract(canopyAbi as AbiItem[], '0x0e8DEF110b1fC3F649ebCd21871E8bfb2f14244D');
...
function processDeposit(event) {
  ...
  auroraAccount.signTransaction({
    ...
    (signedTx) => {web3Aurora.eth.sendSignedTransaction(signedTx.rawTransaction).then(console.log)}
  )
  nonce += 1;
}

vaultContract.events.Deposit(vaultOptions)
    .on('data', event => processDeposit(event))
